<x-layout>
        ...
        <table class="table table-xs md:table-md">
            ...
            <thead class="bg-base-200">
                <tr>
                    ...
                    <th class="text-center">Perbulan</th>
            ...
            <tbody>
                ...
                @foreach ($data as $row)
                    <tr>
                        @php
                            $total_perbulan += $row->perbulan;
                            $total_pertahun += $row->pertahun;
                        @endphp
                        <th class="text-center">{{ $no++ }}</th>
                        <td class="text-center">{{ $row->jenis }}</td>
                        <td class="text-center">Rp{{ number_format($row->perbulan, 2, ',', '.') }}</td>
                        <td class="text-center">Rp{{ number_format($row->pertahun, 2, ',', '.') }}</td>
                ...
                @endforeach
                    <tr>
                      ...
                      <td></td>
                      <td class="text-center font-bold">Rp {{ number_format($total_perbulan, 2, ',', '.') }}</td>
                      <td class="text-center font-bold">Rp {{ number_format($total_pertahun, 2, ',', '.') }}</td>
                    </tr>
    ...
    <dialog id="my_modal_2" class="modal">
        <div class="modal-box">
            <form class="card-body" method="POST" action="{{ url('/biaya-penyimpanan') }}">
                @csrf
                ...
                <div class="my-5 max-w-sm">
                    <label for="input-label-with-helper-text" class="block text-sm font-medium mb-2 ">Perbulan</label>
                    <input name="perbulan" required type="number" step=any id="input-label-with-helper-text"
    ...
    @foreach ($data as $item)
        <dialog id="edit_{{ $item->id }}" class="modal">
            <div class="modal-box">
                <form class="card-body" method="POST" action="{{ url("/biaya-penyimpanan/$item->id") }}">
                    @csrf @method('PATCH')
                    ...
                    <div class="my-5 max-w-sm">
                        <label for="input-label-with-helper-text"
                            class="block text-sm font-medium mb-2 ">Perbulan</label>
                        <input name="perbulan" required type="number" step=any id="input-label-with-helper-text"
                            class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none"
                            aria-describedby="hs-input-helper-text" value="{{ $item->perbulan }}">
                    </div>
    ...
    @foreach ($data as $item)
        <dialog id="delete_{{ $item->id }}" class="modal">
            <div class="modal-box">
                ...
                <form action="{{ url("/biaya-penyimpanan/$item->id") }}" method="POST">
                    @csrf @method('delete')
